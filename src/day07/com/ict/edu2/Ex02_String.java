...
public class Ex02_String {
	public static void main(String[] args) {
		...
		// 1. charAt(int index) : char  (숫자를 받으면 문자로 돌려준다)
		//    위치정보 = index  (index =0부터 길이 -1)
		...
		// 모두 대문자로 만들자 (대문자는 그대로, 소문자는 -32 대문자로 변경 +32)
		...
		System.out.println(s4.contains("res"));
		System.out.println("===================");
		...
		System.out.println(String.format("%.3f", s10)); // 반올림
		System.out.println(String.format("%.1f", s10)); // 소수점 처리
		...
		for (int i = 0; i < 4; i++) {
			System.out.println((char)(b1[i])); //char로 형변환을 해주면 숫자가 문자로 바뀐다.
		}
		...
		// 문자열일 때 == (같다라는 표시)가 주소가 같냐?
		System.out.println(a2 == a3);
		// 문자열일 때 equals (내용이 같냐?) String은 무조건 equals를 써야한다.
		...
		// 'er'를 찾아라 (단어를 찾을 경우는 단어의 첫글자가 시작위치이다.)
		...
		System.out.println(a4.indexOf('e', 5)); // 첫번째로부터 나온 수 더하기 1
		...
		// 12. length() : int => 문자열의 길이(갯수) => 1 부터 시작
		...
		// 예) 홍길동 => 홍*동, 선우은숙 => 선**슥
		String a8 = "홍길동";
		int t1 = a8.length();     // 홍길동은 길이가 3 인덱스는 2
		String res = "";          // 글자 한개씩 뺴내어 써라
		for (int i = 0; i < t1; i++) {
			char s = a8.charAt(i);
			if (i==0 || i==t1-1) {
				res += s;
		...
		a8 = "010-9732-9110";
		int t2 = a8.length();
		int t3 = a8.lastIndexOf("-"); // 마지막 - 의 위치를 찾는다.
		res = "";
		 for (int i = 0; i < t2; i++) {
			 char s = a8.charAt(i);
			 if(i>=0 && i<=t3) {
		...
		a8 = "010-9732-9110";
		t2 = a8.length();
		t3 = a8.indexOf("-");         // 첫번째 - 의 위치를 찾는다.
		int t4 = a8.lastIndexOf("-"); // 두번째 - 의 위치를 찾는다.
		...
		// regex => 구분자(나눌 기준,('/'슬라이스라고도 부른다)쪼갠다), limit => 배열의 크기 (방의 갯수)
		t5 = "사과, 딸기, 망고, @오렌지, 두리안, 용과";  //t5라는 방에 과일들이 들어있다
		String[] arr = t5.split(",");        // 방이 여러개가 생긴다. 방이 여러개 있으면 방번호가(인덱스)가 있다. 인덱스의 번호는 0부터 시작
...
//		System.out.println(arr[5]);               
		...
		String[] arr2 = t5.split("," , 3);        // limit 주면 방의 갯수가 정해지기 때문에
		for (int i = 0; i < arr2.length; i++) {   // 방의 갯수가 적으면 나머지는 마지막방에 다 들어가게 된다.
		...
		String t7 = t6.substring(4);  // 4부터 끝까지
		...
		// beginIndex 위치부터 endIndex 위치까지 (endIndex 위치는 포함하지 않는다.) 중간자리
		// 뒤에숫자에서 앞의숫자를 빼면 그 결과의 수를 추출 숫자가 된다.
		...
		String t9 = "****";
		String t10 = t8.replace(t8.subSequence(4, 8), t9);
		...
		// 19. trim() : String
		// 해당 무나열의 앞, 뒤 공백 제거, 글자사이의 공백은 제거못함
		...
		//     starsWith(String prefix, int toffset)(접두어) : boolean
		//     주어진 문자로 시작하는지 검색, toffset => 시작위치
		
		//     endsWith(String suffix)(접미어) :boolean
		//     주어진 문자로 끝났는지 검사
		...
		// 21. valuOf(기본형자료형) : 들어온 기본 자료형을 String을 변경 시킴 
		// 들어오는거 상관없이 문자로 들어온다
		// 기본자료형-문자(char),논리, 정수 , 숫자
		boolean bo1 = true;
		char ch1 ='a';
		int num1 = 34;
		long num2 = 34L;
		...
		// 22-1. "true" , "false" => true,false : Boolean.paeseBoolean()
		String msg = "true";
		...
		// 22-2. "142", "25" => 142, 25 => Integer.arseInt()
		msg="142";
		...
		byte[] result = msg.getBytes();   // 한글, 한자 등 불가능
		...
		// String.toCharArray() => char[] 모든문자 처리 가능 (전세계 모든 문자)
