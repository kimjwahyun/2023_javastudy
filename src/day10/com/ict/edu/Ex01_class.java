...
public class Ex01_class {
	...
	//                  기능 = 동작 = 멤버 메서드
	//                  생성자(Constructor)= 클래스가 객체로 만들어진때 한번 호출 되서 데이터를 초기화 할 수 있는 기능을 가졌다.
	//                  ** 클래스이름 = 저장이름 = 파일이름 = 생성자이름
	
	// 클래스 구조 : 헤더, 멤버필드, 생성자, 멤버메서드
	// 1. 헤더 : [접근 제한자][클래서 종류에 대한 예약어] class 클래스이름 {     }
	//   1)접근제한자 : 클래스, 멤버필드,멤버메서드, 생성자 모두 처음 사용할 수 있다.
	...
	//            첫글다 소문자,  두 단어 이상일때는 단어 첫 글자들은 대문자
	//            숫자는 중간이나 뒤에 사용, 공백은 사용 불가            
	...
	//  1) 구성 : [접근제한자] [메서드종류] 반환형 메서드 이름([인자]=매개변수){  실행할 내용  }
	//    가) 접근제한자 : public, protected, 생략, private ;  우선순위가 제일 높다
	...
	//                만약에 결과를 가지고 가지 않으면 void라는 예약어를 사용 한다.
 	//  2)메서드이름 : 변수와 같음, 첫글자 소문자, 두 단어 이상일때 단, 첫글자들은 대문자
	//             숫자는 중간이나 뒤에 사용, 공백은 사용 불가
	...
	//     변수가 private 이면 직접 접근을 할 수 없다. = getter()/setter()를 이용해서 접근한다. (우회도로)
	
	// 4. 생성자 : **클래스가 객체로 만들어질 때 한번 호출 된다.
	//    1) 목적 : 멤버필드 (변수,상수)의 초기갖ㅅ을 지정하는 것이 목적이다.
	//    2) 형식 : 클래스 이름과 생성자 이름은 같다. (무조건)
	//            반환형이 없는 메서드의 형태이다.
	...
	//  1) instance(인스터스) 필드, 메서드 : 객체가 생설될 때 같이 생성되는 필드와 메서드
