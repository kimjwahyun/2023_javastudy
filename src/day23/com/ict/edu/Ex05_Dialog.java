...
public class Ex05_Dialog extends JFrame{
	...
	public Ex05_Dialog() {
		...
		jb1.addActionListener(new ActionListener() {
			...
			// 메세지 다이얼 로그 : 사용자에게 정보를 알리는 역할만 한다.(js(자바스트립에서는 alert())
			...
			// JOptionPane.showMessageDialog(parentComponent, message, title, messageType); 메세지타입이랑 방가방가 옆에 있는 삼각형의 느낌표 그림
			// messageType : ERROR_MESSAGE, INFORMATEION_MESSAGE(기본), WARNING_MESSAGE(경고),
			//               QUESTION_MESSAGE, PLAIN_MESSAGE(없음)
			JOptionPane.showMessageDialog(jp, "방가방가", "MASSGE 창", JOptionPane.PLAIN_MESSAGE);
			}
		});
		...
		jb3.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
			// 입력 다이얼 로그 : 사용자에게 정보를 받는 다이얼 로그 (js(자바스트립에서는 prompt()== 거의 사용 안함)
			// String msg = JOptionPane.showInputDialog(parentComponent, message);
			// String msg = JOptionPane.showInputDialog(jp, "나이 : "); // 문자, 숫자 다 나온다.
